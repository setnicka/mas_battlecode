...
public abstract class Bot {
	...
	protected Direction move(Direction dir) throws GameActionException {
		Direction[] chosen_directions = { dir, dir.rotateLeft(), dir.rotateRight() };

		for (Direction d : chosen_directions) {
			if (rc.canMove(d)) {
				rc.move(d);
				return d;
			}
		...
		// pokud necisti rubble, tak je blokovan treba vlastnimi jednotkami -> zkusime zvolit smer nahodne
		Direction new_dir = directions[rand.nextInt(8)];
